...
LETTER : ('A'..'Z' | 'a'..'z') ;
TEXT : (LETTER | '0'..'9' | SPACES | ',' | '.' | '"' | ':' | '?' | '!' | '%' | '/' )+;

END_O : ('#OIC' | '#oic') SPACES?;
END_M : ('#MKAY' | '#mkay')  SPACES?;


language :  ('#HAI' | '#hai') SPACES? comment? head? (TEXT | paragraph | bold | sound | video)* comment? SPACES? ('#KTHXBYE' | '#kthxbye') ;
...
var_create : '#I HAZ' '#IT IZ' TEXT END_M ;
var_read: '#LEMME SEE' LETTER+ END_M;